  ...
  <div class="hello w100">
    <div class="con w100">
      ...
      <div class="linePoint" id="linePoint">
        <div class="points">
          <!--绿色底 point-->
          <div class="point" v-for="(line1, index) in timeLineFun(msgFather)" :key="index" :class="{redBlcak: line1.type === 'CONF_OCCUPY', greenBlcak: line1.type === 'CONF_SCHEDULE', darkBlcak: line1.type === 'CONF_FINISH', orangeBlcak: line1.type === 'CONF_APPROVAL', blueBlcak: line1.type === 'CONF_APPLY'}">
          </div>
        </div>
        <!--橙色占用 灰色不占用-->
...
export default {
  ...
  methods: {
    ...
    timeLineFun (arr) {
      ...
      let timeLine = []
      for (let i = 0; i <= 21; i++) {
        let timeStart = i !== 0 ? startTime + 30 * 60 * 1000 * i : startTime
        let timeEnd = timeStart + 30 * 60 * 1000
        let temp = {
          time: timeStart,
          type: 'none'
        }
        ...
        for (let j in arr) {
          let start = arr[j].startTime
          let end = start + (arr[j].durationSeconds * 60 * 1000)
          if ((start < timeStart || start === timeStart) && (timeEnd < end || timeEnd === end)) { // 包含 取等
            temp.type = arr[j].status
          } else if (timeStart < start && timeEnd > start && timeEnd < end) { // 右交集 不等
            temp.type = arr[j].status
          } else if (timeStart > start && timeStart < end && end < timeEnd) { // 左交集 不等
            temp.type = arr[j].status
          }
        }
        timeLine.push(temp)
